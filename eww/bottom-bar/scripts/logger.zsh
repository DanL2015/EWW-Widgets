...
function make_literal() {
  ...
  [[ "$caches" == "" ]] \
    && print '(box :class "notifications-empty-box" :height 480 :orientation "v" :space-evenly "false" (image :class "notifications-empty-banner" :valign "end" :vexpand true :path "assets/no-notifications.png" :image-width 200 :image-height 200) (label :vexpand true :valign "start" :wrap true :class "notifications-empty-label" :text "No Notifications"))' \
    || print "(scroll :height 480 :vscroll true (box :orientation 'v' :class 'notification-scroll-box' :spacing 20 :space-evenly 'false' $caches))"
}
